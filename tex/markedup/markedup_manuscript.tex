...
%DIF DEL discussion_paper.tex      Tue Jun  4 20:34:05 2019
%DIF ADD landscape_evolution.tex   Fri Jun  7 17:56:12 2019
...
%DIF > HM we can write $\varepsilon_g=c~\varepsilon$ where c[kg] to keep the diffusion coef units the same as in literature
...
%DIF > HM  we need to conclude this with reference to Thaxton? about delta z and stability of the solution
%DIF > and/or that t1 and t2 can be different 
...
%DIF <  simwe intro
SIMWE \DIFdelbegin \DIFdel{-- the Simulation of 
Water Erosion model -- 
...
}\DIFaddend % shallow water flow eq.
\DIFdelbegin \begin{displaymath}
%DIFDELCMD < \label{eq:water}%%%
...
}\begin{displaymath}%DIFDELCMD < \label{eq:sedflow} %%%
q_s = \rho_s ~ q
\end{displaymath}
{\DIFdel{\small
\noindent
...
is approximated by
the steady state sediment flow equationwith diffusion }\DIFdelend : %DIF <  erosion-deposition equation
\DIFdelbegin \begin{displaymath}%DIFDELCMD < \label{eq:sediment} %%%
d_s = 
...
%DIFDELCMD < %%%
%DIF <  ------------------------------------------------------------------------------
\DIFdelend %DIF > (Fig.~\ref{fig:models}b)
%DIF > 
...
is a function of
the change in time , the }\DIFdelend %DIF > The sediment flow rate is computed as weighted particle densities 
...
to simulate the settling of sediment particles.The simulated change in elevation $\Delta z$ %DIF < $\Delta z(x,y,t)$
...
\DIFaddend % Topographic sediment transport factor
...
\DIFaddend % Sediment flow at transport capacity
\noindent
...
\begin{figure}
\center
\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\textwidth,height=0.925\textheight,keepaspectratio]{figures/simulations.pdf}
%DIFDELCMD < %%%
...
\DIFdelbeginFL %DIFDELCMD < %DIFDELCMD < \label{fig:simulations}%%%
%DIFDELCMD < %%%
...
(b) erosion-deposition }[\unit{kg~m}\DIFaddFL{$^{-2}$}\DIFaddendFL ~\DIFdelbeginFL %DIFDELCMD < \unit{mm~hr}%%%
\DIFdelendFL \DIFaddbeginFL \unit{s}\DIFaddendFL $^{-1}$\DIFaddbeginFL ]
\DIFaddFL{for the Study Subwatershed.
The (c) net difference }[\unit{m}] \DIFaddFL{and (d) landforms
for Drainage Area 1.
}\DIFaddendFL }
\DIFdelbeginFL %DIFDELCMD < \label{fig:simwe_simulations}
%DIFDELCMD < %%%
...
%DIF > It is challenging to quantitatively assess the performance of
%DIF > landscape evolution models at a watershed scale.
%DIF > While geomorphological statistics 
%DIF > like slope-area relationships and catchment hypsometry
%DIF > are used to assess model results \citep{Tucker2010}, 
%DIF > the drainage networks generated 
%DIF > may have similar statistics, 
%DIF > but very different spatial patterns \citep{Ijjasz1992}.
%DIF > Furthermore drainage networks 
%DIF > are sensitive to initial conditions
%DIF > with errors propagating exponentially over time
%DIF > \citep{Ijjasz1992}. 
%DIF > Measured versus simulated erosion and deposition values
%DIF > can be compared through landscape element classes 
%DIF > based on slope and contributing area
%DIF > \citep{Peeters2006,Temme2011}.
