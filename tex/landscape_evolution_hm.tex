...
%HM to some extent it is used also in RUSLE2 (see the website and science doc
...
%HM check for redundancy - if we already say this before, maybe it should be just here or we don't need it elswhere?
%HM it is OK to mention it twice but probably not more than that.
...
%HM we already have these variables defined
%{\small
%\noindent
%where: \\
%\noindent
...
%\hspace*{0.5em} ${\bf q_s}$ is the sediment flow rate per unit width (unit sediment load) [$\unit{kg~m^{-1}~s{-1}}$]\\
...
\noindent
where: \\
% we already defined q before \hspace*{0.5em} $\bf q$ is the water flow direction and rate per unit width [$\unit{m^2~s^{-1}}$]\\
...
the sediment-transport capacity $T_c $ as functions of shear stress and stream power respectively. 
...
\hspace*{0.5em} $\gamma=\rho_w~g~h~\sin \beta$ is the shear stress [$\unit{Pa=kg~m^{-2}}$] \\
...
Sediment-transport capacity is computed as a function of the unit stream power $\omega$ \citep{Moore1986}:
\smallskip
\begin{equation}
...
\hspace*{0.5em} $m$ and $p$ is an empirical exponent
}
...
%HM I found the statement below confusing - does this refer to r.sim.terrain or the old USLE?
...
PLEASE ADD REFS (US, Greece, others).
...
assuming that the sediment flow is equal sediment transport capacity: 
...
%Landscape evolution
\noindent
UPDATE based on the new structure of the paper
