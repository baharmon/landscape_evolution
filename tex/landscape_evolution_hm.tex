...
Shallow water flow ${\bf q}$ can be approximated by
...
is found by Green's function Monte Carlo path sampling method
where the steady-state solution is obtained by accumulation of the evolving source
over the given time period. This accumulation process can be also interpreted as
...
$ \nabla^2 [h^{5/3}]$
into the solution of the continuity 
????and momentum ????
equations for steady state water flow:
...
\noindent
 where: \\
 \noindent
 \hspace*{0.5em} $\varepsilon$ [$\unit{m}^{4/3}~\unit{s}^{-1}$] is a spatially variable diffusion coefficient. \\
}
...
The initial number of particles per grid cell is proportinal to rainfall excess rate $i_e$ and 
the particles are then routed across the landscape by finding a new position of the walker at time $t + \Delta t$ by:
...
Similarly as for the shallow water flow we can solve the steady state sediment flow 
equation using Greens function Monte Carlo path sampling, but in this case each particle will 
have weight dependent on the rate of erosion or deposition.

A simplified, steady-state form of the continuity equation can be derived
for the conditions when, at the given location, the change in water flow
and sediment concentrations over time is close to zero.  The net erosion or deposition
rate $d_s(x,y)$ is then computed as a divergence of steady state sediment flow
rate per unit width ${\bf q_s}(x,y)$:

\smallskip
\begin{equation}
\label{eq:steady_sed}
{\partial [\rho_s(x,y,t)h(x,y,t)] \over \partial t}=0.
\quad \longrightarrow \quad
\nabla\cdot {\bf q_s}(x,y) = d_s(x,y) 
\end{equation}
\smallskip
\noindent
Assuming steady rainfall excess rates,  equation (\ref{eq:steady_sed})
applies to the peak flow and peak concentration conditions when
water depth and sediment concentrations remain constant over time.
