...
\noindent
Gravitational diffusion is applied to the changed topography during each landscape evolution time step
...
\end{equation}
where $z_{t+ 1/2 \Delta t}$ is elevation after the change $\Delta z_s$ due to net erosion or deposition (Eq.~\ref{eq:evolution}),
$z_{t+\Delta t}$ is elevation after the eroded elevation surface
was smoothed by diffusion by adding the diffusion driven elevation change $\Delta z_g$, (Eq.~\ref{eq:grav_diffusion}).  
...
\end{equation}
and the net erosion and deposition rate $d_s$ can be expressed as proportional to the difference between
...
\end{equation}
\noindent
This concept is used in several well established erosion models such as Water Erosion Prediction Project (WEPP)
\citep{Flanagan2013} or SIMWE \citep{Mitas1998}. 
%HM also RUSLE2???? find the correct name and reference to RUSLE with erdep.
