...
\citep{Mitasova2013}: (DO WE HAVE OTHER REFERENCE?)
...
HOW IS THE TIME handled here? Does the application of grav diffusion move time step another delta t 
or is it applied within the same time step? 
If it is the same time step we have
\begin{equation}
\label{eq:evolution_disc1} 
z_{t+\Delta t} = z_t + \Delta z_s + \Delta z_g 
\end{equation}
if the time step advances we need two equations, something like this:
\begin{equation}
\label{eq:evolution_disc1} 
...
${\bf q_s} = \varrho~{\bf s_0}$ and then net eroion-deposition $d_s$ 
...
% Erosion regime
This model can simulate broad range of erosion regimes at each landscape evolution time step
...
elevation change is derived from non-steady state flow...
