...
\begin{figure}%[H]
\center
\includegraphics[width=\textwidth,height=0.925\textheight,keepaspectratio]{figures/fig01.pdf}
\caption{
...
r.landscape.evol \citep{Barton2010},
and eSCAPE \citep{Salles2019}
...
\begin{figure}%[H]
\center
\includegraphics[width=\textwidth,keepaspectratio]{figures/fig02.pdf}
...
or the Unit Stream Power Erosion Deposition (USPED) model (Fig.~\ref {fig:fig02}).  
...
\begin{table*}
\small
\caption{Examples of geospatial soil erosion models}
...
\begin{figure}
\center
\includegraphics[width=\textwidth,height=0.95\textheight,keepaspectratio]{figures/fig03.pdf}
\caption{Water and sediment flows modeled with spatially variable landcover
for Patterson Branch, Fort Bragg, NC:
(a) water depth [m] simulated by SIMWE 
for a 10~\unit{min} event with 50~\unit{mm~hr}$^{-1}$ 
in the subwatershed;
(b) flow accumulation for RUSLE3D in the subwatershed;
(c) erosion and deposition [\unit{kg~m}$^{-2}$~\unit{s}$^{-1}$] 
simulated by SIMWE in drainage area 1; and
...
and the ratio of the sediment flow rate $q_s = |\vec{q_s}|$ 
...
The simulation modes in r.sim.terrain include (Fig.~\ref{fig:fig02}):
\begin{itemize}
...
% using the GRASS GIS module r.slope.aspect (see the equations in \citet{hofierka2009}),
...
The sediment flow equation (Eq.~\ref{eq:steady_sed}),
...
= d_s
%\sigma(\vec{r}) T(\vec{r})=Dc/T * T=d_S
\end{equation}
{\small
\noindent
where:\\
...
On the left hand side of Eq.~(\ref{eq:difsedim})
...
and the water flow velocity (Eq.~\ref{eq:sedflow}) and 
%$\vec{q_s} = \varrho~\vec{s_0}$.  %VERIFY 
the net erosion-deposition rate $d_s$ 
is computed as the divergence of sediment flow using Eq.~(\ref{eq:steady_sed}).
...
multiplied by grid cell width (Fig.~\ref{fig:fig03}d). 
...
and the slope (Fig.~\ref{fig:fig03}e). 
\begin{equation}
\label{eq:ls_factor}
...
is derived from Eq.~(\ref{eq:evolution}) for landscape evolution
and then Eq.~(\ref{eq:grav_diffusion})
for gravitational diffusion.
...
\begin{figure}
\center
\includegraphics[width=\textwidth,height=0.95\textheight,keepaspectratio]{figures/fig04.pdf}
...
%
The study landscape 
-- a subwatershed of Patterson Branch (Fig.~\ref{fig:fig04}) 
...
(Fig.~\ref{fig:fig05} \& \ref{fig:fig06}). 
%
...
(Fig.~\ref{fig:fig05}c \& \ref{fig:fig06}c) --
...
\begin{figure}
\center
\includegraphics[width=\textwidth,height=0.95\textheight,keepaspectratio]{figures/fig05.pdf}
\caption{Morphological change in the subwatershed of Patterson Branch, Fort Bragg, NC, USA:
(a) landcover in 2014, 
(b) landforms in 2012,
...
\begin{figure}
\center
\includegraphics[width=\textwidth,height=0.95\textheight,keepaspectratio]{figures/fig06.pdf}
\caption{Detailed morphological change 
...
\begin{figure}
\center
\includegraphics[width=\textwidth,height=0.925\textheight,keepaspectratio]{figures/fig07.pdf}
...
for the subwatershed in the final 3~\unit{min} timestep;
(c) net difference [\unit{m}] and (d) landforms 
for drainage area 1.
}
\label{fig:fig07}
\end{figure}
...
\begin{figure}
\center
\includegraphics[width=\textwidth,height=0.925\textheight,keepaspectratio]{figures/fig08.pdf}
...
\begin{figure}
\center
\includegraphics[width=\textwidth,height=0.95\textheight,keepaspectratio]{figures/fig09.pdf}
\caption{
Steady state SIMWE simulations 
for a 120~\unit{min} event 
with a rainfall intensity of 50~\unit{mm~hr}$^{-1}$
for Patterson Branch, Fort Bragg, NC:
(a) depth of unsteady flow [\unit{m}] and
(b) erosion-deposition [\unit{kg~m}$^{-2}$~\unit{s}$^{-1}$]
...
\url{https://doi.org/10.5281/zenodo.3243699}.
...
with the DOI:
\url{https://doi.org/10.5281/zenodo.3243700}.
The
