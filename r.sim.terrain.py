...
def main():
    ...
    if not runoff:
        runoff = 'runoff'
        gscript.mapcalc(
            "runoff = {runoff_value}".format(**locals()),
            overwrite=True)

    if not mannings:
        mannings = 'mannings'
        gscript.mapcalc(
            "mannings = {mannings_value}".format(**locals()),
            overwrite=True)

    if not detachment:
        detachment = 'detachment'
        gscript.mapcalc(
            "detachment = {detachment_value}".format(**locals()),
            overwrite=True)

    if not transport:
        transport = 'transport'
        gscript.mapcalc(
            "transport = {transport_value}".format(**locals()),
            overwrite=True)

    if not shearstress:
        shearstress = 'shearstress'
        gscript.mapcalc(
            "shearstress = {shearstress_value}".format(**locals()),
            overwrite=True)

    if not mass:
        mass = 'mass'
        gscript.mapcalc(
            "mass = {mass_value}".format(**locals()),
            overwrite=True)
    ...
    if density_raster:
        ...
        gscript.mapcalc(
            "density = {density_raster} * 1000".format(**locals()),
            overwrite=True)
    else:
        # convert g/cm^3 to kg/m^3
        gscript.mapcalc(
            "density = {density_value} * 1000".format(**locals()),
            overwrite=True)

    if not c_factor:
        c_factor = 'c_factor'
        gscript.mapcalc(
            "c_factor = {c_factor_value}".format(**locals()),
            overwrite=True)

    if not k_factor:
        k_factor = 'k_factor'
        gscript.mapcalc(
            "k_factor = {k_factor_value}".format(**locals()),
            overwrite=True)
...
class Evolution:
    ...
    def simwe(self, dx, dy, depth):
        ...
        gscript.mapcalc(
            f"{rain} = {self.rain_intensity}*{self.runoff}",
            overwrite=True)
    ...
    def event_based_r_factor(self):
        ...
        gscript.mapcalc(
            f"{rain_energy}" \
            f"=0.29*(1.-(0.72*exp(-0.05*{self.rain_intensity})))",
            overwrite=True)
    ...
    def gravitational_diffusion(self, evolved_elevation):
        ...
        * divergence (m^-1))
        ...
        # update elevation
        gscript.mapcalc(
            f"{evolved_elevation} = {settled_elevation}",
            overwrite=True)
        gscript.run_command(
            'r.colors',
    ...
    def fill_sinks(self, evolved_elevation):
        ...
        # update elevation
        gscript.mapcalc(
            f"{evolved_elevation} = {depressionless_elevation}",
            overwrite=True)
        gscript.run_command(
            'r.colors',
            map=evolved_elevation,
    ...
    def compute_difference(self, evolved_elevation, difference):
        ...
        gscript.mapcalc(
            f"{difference} = {evolved_elevation}-{self.elevation}",
            overwrite=True)
        gscript.run_command(
            'r.colors',
            map=difference,
    ...
    def usped(self):
        ...
        gscript.mapcalc(
            f"{ls_factor}=({depth}^{self.m})*(sin({slope})^{self.n})",
            overwrite=True)
        ...
        gscript.mapcalc(
            f"{qsx}={sediment_flux}*cos({aspect})",
            overwrite=True)

        # compute sediment flow rate in y direction (m^2/s)
        gscript.mapcalc(
            f"{qsy}={sediment_flux}*sin({aspect})",
            overwrite=True)
    ...
    def rainfall_event(self):
        ...
        while i < iterations:
            ...
            if self.mode == "simwe_mode":
                ...
                raise RuntimeError(
                    f'{self.mode} mode does not exist')
        ...
        gscript.mapcalc(
            f"{net_difference}={evol.elevation}-{self.elevation}",
            overwrite=True)
        gscript.write_command(
            'r.colors',
            map=net_difference,
